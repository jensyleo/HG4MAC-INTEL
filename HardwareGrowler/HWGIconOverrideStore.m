...
#pragma mark Export/import profile

static NSError *HWGIconProfileError(NSString *description) {
	return [NSError errorWithDomain:@"HWGIconOverrideStore" code:1 userInfo:@{NSLocalizedDescriptionKey: description}];
}

// Runs a command synchronously and returns YES on a clean (status 0) exit.
static BOOL HWGRunTool(NSString *launchPath, NSArray<NSString *> *arguments, NSError **error) {
	...
	if (![task launchAndReturnError:&launchError]) {
		if (error) *error = launchError ?: HWGIconProfileError(@"Couldn't launch helper tool.");
		return NO;
	}
	[task waitUntilExit];
	if (task.terminationStatus != 0) {
		...
		if (error) *error = HWGIconProfileError(errString.length ? errString : @"Helper tool failed.");
		return NO;
	}
	return YES;
}

- (BOOL)exportProfileToURL:(NSURL *)destinationURL error:(NSError **)error {
	if (!destinationURL) {
		if (error) *error = HWGIconProfileError(@"No destination chosen.");
		return NO;
	}
	...
	dispatch_sync(self.queue, ^{
		// -c: create archive, -k: zip format, --keepParent: so unzipping reproduces the
		// "IconOverrides" folder name itself (matters for import, which looks for it by name).
		success = HWGRunTool(@"/usr/bin/ditto",
							 @[@"-c", @"-k", @"--keepParent", [self storeDirectory].path, destinationURL.path],
							 &innerError);
	});
...
- (BOOL)importProfileFromURL:(NSURL *)sourceURL error:(NSError **)error {
	if (!sourceURL) {
		if (error) *error = HWGIconProfileError(@"No file chosen.");
		return NO;
	}

	NSURL *tempDir = [[NSFileManager defaultManager] URLForDirectory:NSItemReplacementDirectory
															  inDomain:NSUserDomainMask
													 appropriateForURL:[self storeDirectory]
																create:YES
																 error:error];
	if (!tempDir) return NO;

	NSError *unzipError = nil;
	if (!HWGRunTool(@"/usr/bin/ditto", @[@"-x", @"-k", sourceURL.path, tempDir.path], &unzipError)) {
		if (error) *error = unzipError;
		[[NSFileManager defaultManager] removeItemAtURL:tempDir error:nil];
		return NO;
	}

	NSURL *extractedStoreDir = [tempDir URLByAppendingPathComponent:@"IconOverrides"];
	if (![[NSFileManager defaultManager] fileExistsAtPath:extractedStoreDir.path]) {
		if (error) *error = HWGIconProfileError(NSLocalizedString(@"That file doesn't look like an icon profile.", @""));
		[[NSFileManager defaultManager] removeItemAtURL:tempDir error:nil];
		return NO;
	}
	...
	__block NSError *innerError = nil;
	dispatch_sync(self.queue, ^{
		NSURL *liveStoreDir = [self storeDirectory];
		[[NSFileManager defaultManager] removeItemAtURL:liveStoreDir error:nil];
		success = [[NSFileManager defaultManager] moveItemAtURL:extractedStoreDir toURL:liveStoreDir error:&innerError];
		if (success) {
			self.overriddenNames = [[self loadIndexFromDisk] mutableCopy] ?: [NSMutableSet set];
			[self.decodedImageCache removeAllObjects];
		}
